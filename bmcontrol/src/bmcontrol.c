// original code: https://www.olimp-z.ru/products/MP707/main.c
...
int scan() {
    ...
    for(int i=1;i<=ONEWIRE_COUNT;i++) {
        printf("sensor %d = %08x%08x\n", i, (ONEWIRE_ROM[i-1]>>32)&0xFFFFFFFF, ONEWIRE_ROM[i-1]&0xFFFFFFFF);
    }
    return 1;
}

/* experimental, need test */
int tempall() {
    int ret;
    int ret2;
    ret = SEARCH_ROM(0, 0);
    for(int i=1;i<=ONEWIRE_COUNT;i++) {
//            printf("%08x", (ONEWIRE_ROM[i-1]>>32)&0xFFFFFFFF);
//            printf("%08x = ",ONEWIRE_ROM[i-1]&0xFFFFFFFF);
            printf("",ONEWIRE_ROM[i-1]&0xFFFFFFFF,"\n");
            
            ret2 = GET_TEMPERATURE(ONEWIRE_ROM[i-1], T);
            if(ret2) { printf("%f\n",T); }			
    }
    return 1;
}
        ...
        if(argc==1) {
            ...
            printf("   delay <5-255>                 Set delay time of device before power save\n");
             ...
             if(strcmp(argv[1],"ports") == 0) buf = read_ports();
                  ...
                  tempall();	// experimental, need test
             }
